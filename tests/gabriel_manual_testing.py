...
def fminmax_indx(tree, altitudes, marker, alpha=lambda x: np.exp(1-x)):
    ...
    for i in tree.root_to_leaves_iterator(include_root=False):
        ...
        dist_sib = min_alpha_marker[sibling[i]]*(1+altitudes[parents[i]])
    ...
    for i in range(tree.num_leaves()):
        if(min_alpha_marker[i] <= output[i]):
            output[i] = min_alpha_marker[i]
            output_idx[i] = i
...
for i in range(10):
    ...
    mid_time = time.time()
    cpp_return = m.fminmax_idx(tree, np.array(altitudes), np.array(mo))
